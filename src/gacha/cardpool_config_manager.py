...
class CardPoolConfig:
    ...
    rate_up_items: Dict[str, Any]  # UP物品配置
    included_items: Dict[str, List[str]]  # 包含物品配置
    probability_progression: Dict[str, Dict[str, Any]]  # 概率递增配置
    pool_config: Optional[Dict[str, float]] = None  # 卡池特定配置
...
class CardPoolConfigManager:
    """卡池配置管理类
    
    负责卡池配置文件的加载、保存和CRUD操作
    """
    
    def __init__(self, config_dir: str = "config/banners"):
        """初始化卡池配置管理器
        ...
        self._configs: Dict[str, CardPoolConfig] = {}  # 内存中的配置数据，键为配置名称
    ...
    def get_config(self, config_name: str) -> CardPoolConfig:
        """获取指定配置
    ...
    def add_config(self, config_name: str, config_data: Dict[str, Any]) -> CardPoolConfig:
        """添加新配置文件
        ...
        if config_name in self._configs:
            logger.error(f"配置已存在: {config_name}")
            raise ValueError(f"配置 {config_name} 已存在")
        ...
        try:
            ...
            # 更新时间戳
            config_instance.created_at = datetime.now().isoformat()
            config_instance.updated_at = datetime.now().isoformat()
            ...
            logger.info(f"已添加配置: {config_name}")
            return config_instance
        except Exception as e:
            logger.error(f"添加配置失败: {config_name} - {e}")
            raise RuntimeError(f"添加配置 {config_name} 失败: {e}")
    ...
    def update_config(self, config_name: str, config_data: Dict[str, Any]) -> CardPoolConfig:
        ...
        try:
            ...
            # 保留创建时间
            config_instance.created_at = self._configs[config_name].created_at
            # 更新时间戳
            config_instance.updated_at = datetime.now().isoformat()
    ...
    def reload_config(self, config_name: str) -> CardPoolConfig:
        ...
        try:
            with open(file_path, 'r', encoding='utf-8') as f:
                config_data = json.load(f)
                config_instance = PoolConfig.from_dict(config_data)
                self._configs[config_name] = config_instance
...
# 创建全局配置实例
card_pool_config_manager = CardPoolConfigManager()
