...
from .cardpool_config_manager import CardPoolConfig
from ..item_data.item_data_manager import ItemDataManager
...
class GachaMechanics:
    ...
    @staticmethod
    def caculate_rate_5star(rate_number: int, pool_config: CardPoolConfig) -> float:
        """
        计算当前抽卡的五星概率
        参数：
    ...
    @staticmethod
    def caculate_rate_4star(rate_number: int, pool_config: CardPoolConfig) -> float:
        """
        计算当前抽卡的四星概率
        参数：
        ...
        if rate_number < hard_pity_pull:
            ...
            logger.error(f'四星保底计数错误：{rate_number}')
        return local_rate
    ...
    def execute_pull(
        ...
        # 获取包含物品配置
        included_items = pool_config.included_items

        # 根据包含的物品，筛选出允许的物品
        rate_up_5star = pool_config.rate_up_items.get('5star', [])
        rate_up_4stars = pool_config.rate_up_items.get('4star', [])
        
        # 获取包含物品的详细信息, 主要为稀有度, 和类型
        item_details = {}
        for rarity in included_items:
            for item_name in included_items[rarity]:
                item_details[item_name] = ItemDataManager.get_item_details(item_name)
        ...
        # >核心抽卡逻辑<
        # 1.检查是否抽到五星物品
        if GachaMechanics.caculate_rate_5star(pity_5star + 1, pool_config) > local_random:
            # 抽到五星，重置五星保底计数
            new_pity_5star = 0
            ...
            # 从物品列表中获取所有五星物品
            five_star_items = [name for name, detail in item_details.items() if detail.get('rarity') == 5]
            ...
            if not five_star_items:
                ...
                new_pity_4star = 0
                # 从四星物品中随机选择
                four_star_items = [name for name, detail in item_details.items() if detail.get('rarity') == 4]
                local_item = random.choice(four_star_items) if four_star_items else "未知物品"
            else:
                # 过滤掉UP物品（rate_up_5star是列表，所以要用not in）
                standard_5star_items = [item for item in five_star_items if item not in rate_up_5star]
                
                if not _5star_guaranteed and random.random() < (1 - up_5star_rate) and standard_5star_items:
                    # 抽到非up五星物品
                    # 设置大保底，下次五星必定为本期限定五星物品
                    _5star_guaranteed = True
                    local_item = random.choice(standard_5star_items)
                else:
                    # 抽到up五星物品
                    # 重置大保底
                    ...
                    if rate_up_5star:
                        local_item = random.choice(rate_up_5star)
                    else:
                        local_item = random.choice(five_star_items)
        # 2.检查是否抽到四星物品
        elif GachaMechanics.caculate_rate_4star(pity_4star + 1, pool_config) > local_random:
            # 抽到四星物品，重置四星保底计数
            new_pity_4star = 0
            # 五星保底计数+1
            new_pity_5star += 1
            ...
            if not four_star_items:
                ...
                new_pity_5star += 1
                # 从三星物品中随机选择
                three_star_items = [name for name, detail in item_details.items() if detail.get('rarity') == 3]
                local_item = random.choice(three_star_items) if three_star_items else "未知物品"
            else:
                # 检测是否抽到了四星角色
                item_type = 'role'
                ...
                # 2.1检测是否抽到了up四星物品
                if rate_up_4stars and random.random() < up_4star_rate:
                    # 抽到概率up四星角色
                    _4star_guaranteed = False
                    # 从UP四星物品列表中随机选择一个
                    up4items = [item for item in rate_up_4stars if item in item_details and item_details[item].get('type') == item_type]
                    if up4items:
                        local_item = random.choice(up4items)
                    else:
                        # 如果没有符合条件的UP物品，从所有四星物品中选择
                        same_type_4star_items = [item for item in four_star_items if item_details[item].get('type') == item_type]
                    ...
                    # 从非UP四星物品列表中随机选择
                    non_up_4star_items = [item for item in four_star_items if item not in rate_up_4stars and item_details[item].get('type') == item_type]
        ...
        # 3.既没抽到五星物品，也没抽到四星物品，则抽取到三星武器
            ...
            # 从物品列表中获取所有三星物品
            three_star_items = [name for name, detail in item_details.items() if detail and detail.get('rarity') == 3]
            # 随机选择一个三星物品
            if three_star_items:
